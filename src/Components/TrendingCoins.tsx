...
const TrendingCoins = () => {
    ...
    const [topTrending, setTopTrending] = useState(trending.coins?.slice(0, 3))
    useEffect(() => {
        setTopTrending(trending.coins?.slice(0, 3))
    }, [trending])
    return (
        <div className="p-3 bg-white border d-flex flex-column  gap-2  rounded  shadow ">
            <h3>Trending Coins (24h)</h3>
            {topTrending?.slice(0, 3)?.map((items: Coins, index: number) => {
                return (
                    <div key={index} className='d-flex justify-content-between'>
                        <div className='d-flex gap-1 align-items-center'>
                            <Image loading="lazy" src={items?.item?.thumb ?? ''} alt='sybmol' width={20} height={20} />
