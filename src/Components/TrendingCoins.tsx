            ...
            {trending?.coins && trending?.coins?.slice(0, 3)?.map((items: Coins, index: number) => {
                return (
                    <div key={index} className='d-flex justify-content-between'>
                        ...
                        {items?.item?.data?.price_change_percentage_24h?.btc && <span className={`justify-content-center ${items?.item?.data?.price_change_percentage_24h?.btc > 0 ? 'green text-green' : 'red text-red'}`}>
